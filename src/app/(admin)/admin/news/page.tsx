...
export default function NewsPage() {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    if (res.ok) {
      ...
      setForm((p) => ({ ...p, coverImage: data.data.url }));
    } else {
      ...
      <div className="p-4 md:p-8">
        ...
        {showForm && (
          <div style={{ background: 'var(--admin-bg-card)', boxShadow: 'var(--admin-shadow)', border: 'none', borderRadius: '12px', padding: '24px', marginBottom: '24px' }}>
            <h3 style={{ color: 'var(--admin-text-primary)', fontSize: '1rem', fontWeight: 600, fontFamily: 'var(--font-outfit), sans-serif', margin: '0 0 20px' }}>{editingId ? 'Edit' : 'Add'} News</h3>
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div><label style={labelStyle}>Title</label><input type="text" value={form.title} onChange={(e) => handleTitleChange(e.target.value)} required style={inputStyle} onFocus={(e) => (e.target.style.borderColor = 'var(--admin-primary)')} onBlur={(e) => (e.target.style.borderColor = 'var(--admin-border)')} /></div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <select value={form.categoryId} onChange={(e) => setForm((p) => ({ ...p, categoryId: e.target.value }))} required style={inputStyle} onFocus={(e) => (e.target.style.borderColor = 'var(--admin-primary)')} onBlur={(e) => (e.target.style.borderColor = 'var(--admin-border)')}>
